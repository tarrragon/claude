...
5. **測試錯誤價值觀鐵律**
   **找到錯誤是有價值的，目標是服務需求而非追求數字**
   - 測試失敗 = 暴露真正問題 = 有價值的發現
   - 100% 通過率不應靠「跳過測試」達成
   - **禁止行為**：為了數字好看而標記 `skip`、移除失敗測試、降低測試標準
   - **正確做法**：分析失敗原因 → 判斷需求優先級 → 修復問題或實作缺失功能
   - **需求優先級判斷**：
     - 核心功能（現在就需要）→ 立即修復，不可跳過
     - 次要功能（未來才需要）→ 可記錄為技術債務，排程處理
   - **案例**：匯入功能測試失敗 → 發現 UI 未實作 → 這是核心功能 → 必須修復
...
**強制流程**：
1. **暫停修復衝動** - 看到測試失敗時，先深呼吸，不要急著修復
2. **建立 Ticket** - 使用 `/ticket-create` 記錄問題
3. **六階段評估** - 使用 `/pre-fix-eval` 完成系統化分析
4. **學習反思** - 問自己：「我們能從這個錯誤學到什麼？」
5. **修復執行** - 完成評估後，分派代理人執行修復
6. **經驗記錄** - 將學習到的模式記錄到 error-patterns
...
**重要警告**：Hook 系統無法檢查 Claude 回覆格式，格式遵循完全依賴自覺。**每次回覆前必須自我檢查格式是否完整**。

**回覆類型判斷**：

| 回覆類型 | 格式要求 | 範例 |
|---------|---------|------|
| **決策/分析/規劃** | 完整 6 欄位 | 問題分析、方案設計、任務規劃 |
| **執行報告** | 完整 6 欄位 | 程式碼修改、測試執行、檔案操作 |
| **簡單確認** | Token + 簡述 | 「已推送」「已提交」「了解」 |
...
Where: origin/main
Why: 用戶要求推送
How: [Task Type: Review] git push origin main
...
Where: Hook 系統和配置檔案
Why: 需要找出機制失效的根本原因
How: [Task Type: Analysis] 探索配置 -> 分析腳本 -> 整理報告
...
- [ ] 是否包含 Where 欄位？
- [ ] 是否包含 Why 欄位？
- [ ] 是否包含 How 欄位（含 Task Type）？
- [ ] 是否有 --- 分隔線後接具體內容？

**如果任何一項未勾選，必須補充完整後才能回覆。**
...
- 所有測試 100% 通過
- 重構評估完成（無論是否執行重構）
- **技術債務已捕獲並建立 Ticket**（執行 `/tech-debt-capture`）
