...
push 以 `git archive HEAD -- .claude` 取 git tracked 樹，再依 `should_exclude` 過濾後複製到遠端 staging；pull 以三方合併處理 `.claude/` 內容。排除分類的 SSOT 為 `.claude/lib/sync_exclude_manifest.py`（`should_exclude` / `LOCAL_ONLY_PATTERNS` / `CREDENTIAL_PATTERNS` 等），push/pull/status 三腳本與 hook 共用，避免漂移（ARCH-020）。
...
- 獨立 Repo: https://github.com/tarrragon/claude.git
- 排除分類 SSOT: `.claude/lib/sync_exclude_manifest.py`
