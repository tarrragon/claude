...
| **候選數量閾值**      | 列出的「假設根因」候選是否 ≥ 5 個？（不是「實作方案」，是「對問題的解釋假設」）| 候選 < 5 → 反模式 1（W 階段不夠廣泛），強制補至 5+ 後再進 R |
...
**Why**：W17-098 第一輪 WRAP 在 W 階段只列出單一假設「runtime hardcoded deny」，在 R 階段宣稱「成本/收益不對稱不執行」直接結束實驗，結論被 W17-099 一個簡單對照實驗推翻。根因是 W 階段把「現有觀察的可能解釋」當「所有可能假設」。

**Consequence**：候選假設不足 5 個會讓 R 階段的對照實驗設計缺乏區辨度，否證任一假設後容易跳到結論而非繼續擴增。

**Action**：

| 任務類型 | 最低候選數 | 強制涵蓋 |
|---------|---------|---------|
| 根因調查（debug / 對照實驗）| 5+ 假設 | 內容粒度 / 結構粒度 / 時間粒度 / 累積粒度 / 框架以外 |
| 架構決策 | 4+ 方案 | 三類涵蓋（新增工具 / 改造既有 / 零工具）|
| 流程改善 | 3+ 方案 | 三類涵蓋 |

**自查問句**：「我列的假設是否『現有觀察的可能解釋』而非『實驗能告訴我什麼』？」前者只是現況描述，後者才是 widen。詳見 W17-104 反模式 1 案例。
...
| **推翻後框架跳脫**（反模式 4 防護）| 上一輪假設被否證後，新假設清單是「修補原框架」還是「跳脫原框架」？ | 候選全在原框架內（同層、同領域、同抽象）→ 強制補「框架以外」候選後再進 A |
...
**Why**：W17-098 第一輪寫「runtime hardcoded deny」，被 W17-099 推翻後仍寫「runtime 偶發性 deny」——只是把絕對結論軟化為「偶發」，沒跳出「runtime 層問題」框架。第一個假設一旦寫入文件，AI 推理會傾向修補它而非完全推翻。

**Consequence**：anchor 在原框架的修補式假設無法擴增實質選項空間，下一輪實驗仍在原框架內驗證，根因偏離真實位置。

**Action**：當任一假設被否證後，下一輪 W 階段必須先執行「框架跳脫自檢」：

| 自檢問句 | 修補式回答（紅旗） | 跳脫式回答（綠旗） |
|---------|----------------|----------------|
| 新假設清單與舊清單在同一抽象層嗎？ | 是（如全在 runtime / 全在 prompt 結構）| 否，新清單跨多個抽象層 |
| 新假設只是把舊假設「軟化 / 加例外 / 縮小範圍」嗎？ | 是（如「hardcoded」→「偶發」）| 否，新假設挑戰舊框架本身 |
| 反面框架被列入候選了嗎？ | 否（候選全在原框架語意內） | 是（含「問題本身可能不存在」「框架以外的解釋」）|
| 若推翻者（用戶/實驗結果）是對的，原框架本身可能錯嗎？ | 沒考慮 | 已考慮並列為候選 |

**強制動作**：3 個以上紅旗 → 回 W 階段，強制補「跨抽象層 + 反面框架 + 框架以外」候選後再進 R。

詳見 W17-104 反模式 4 案例（W17-098 → W17-099 推翻後仍 anchor「runtime 層」）。
...
### 悖論識別（規則 / Skill / Hook 設計時適用）

設計規則 / 流程 / 系統時，需檢查規則自身是否觸發悖論（自我參照、隱性 paternalism、目的-手段倒置、萬能框架幻覺）。完整 5 條檢查清單與 4 條件 benevolent paternalism 測試見 `references/anti-paternalism.md`。

> 此檢查為**設計者場景**專用，一般決策諮詢（個人化建議 / 根因分析 / 提案評估）不需要執行。
...
| 文件 | 內容 |
| ---- | ---- |
| `references/principles/four-villains.md` | Heath 兄弟四大決策 villain 與 WRAP 階段對應；修改階段結構時參考 |
| `references/principles/premortem-klein.md` | Gary Klein premortem 方法的實證依據；修改 P 階段問題形式時參考 |
| `references/principles/voss-influence-transparency.md` | Voss 影響力透明化原則；修改自我暴露範本或反 dark pattern 規則時參考 |
| `references/principles/benevolent-paternalism.md` | 4 條件 benevolent paternalism 測試的學術來源；新增規則時的正當性檢驗依據 |

> 執行 WRAP 不需要讀 `principles/`，主文與執行者參考已足夠。
