...
description: "並行評估決策工具。啟動多個 Agent 從不同視角並行掃描，彙整後只執行值得改的項目。Use for: 重構前掃描冗餘/重複, 架構方案快速評估, 系統設計品質掃描, 結論/報告審查, 任何需要多角度快速評估的決策"
---
...
**Phase 3** → 彙整 + Worth-It Filter（只保留值得行動的項目）
...
**常駐委員**：所有情境自動加入 linux（Good Taste 品質把關），總計 3-4 個 Agent。linux 評分對應 Worth-It Filter：Garbage = 高幅度、Acceptable = 中幅度、Good taste = 無發現。
