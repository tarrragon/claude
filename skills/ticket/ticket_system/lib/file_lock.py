...
# W9-001 將 file_lock() 主路徑遷移至 filelock，但 create_id_allocation_lock /
# _acquire_dir_lock 仍以原生 fcntl.flock 實作，依賴 _HAS_FCNTL 與 fcntl 模組。
# 上游重構移除此定義區塊卻保留下游引用，致 ticket create 觸發 NameError。
# 補回 fcntl gate 使兩條鎖路徑並存（POSIX 用 fcntl，Windows graceful degrade）。
try:
