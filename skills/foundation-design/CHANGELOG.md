...
**Version**: 6.1.0 — 維度與產物表補「適用條件」欄；資料庫／DevOps／可觀測性三列的權威 `saas-tech-selection` 明示為 SaaS/伺服器端專案預設形態，非 SaaS 專案（CLI 工具、函式庫、單機 app）改依既有「改寫產物」機制取可用部分，無可用部分則改指專案自身 CLAUDE.md 技術選型記錄或標「無」+ 理由；不新增第二套權威。實測依據：非 SaaS 專案（如 Go CLI 工具）走此路由會被引導至 managed database／k8s 等不適用選型。
