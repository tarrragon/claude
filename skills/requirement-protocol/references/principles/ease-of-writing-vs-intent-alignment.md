...
> **角色**：本卡是 `requirement-protocol` 的支撐型原則（principle）、被相關抽象層原則段引用、是 [`verification-timeline-checkpoints.md`](./verification-timeline-checkpoints.md) / [`test-first-red-before-green.md`](./test-first-red-before-green.md) / [`url-as-state-container.md`](./url-as-state-container.md) 等多份 principle 共同參照的 meta-principle。
>
