...
> **角色**：本卡是 `requirement-protocol` 的支撐型原則（principle）、被 reference `clarifying-ambiguous-instructions.md`（篩選類指令解法策略）引用、跟 [`filter-instruction-clarification.md`](./filter-instruction-clarification.md)（三問澄清）配對使用。
>
