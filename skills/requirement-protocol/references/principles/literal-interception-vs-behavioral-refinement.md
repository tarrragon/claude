...
> **角色**：本卡是 `requirement-protocol` 的支撐型原則（principle）、被支柱 4「Multi-pass Refinement」與第 7 原則明示引用、是「為什麼用 hook 防 collapse 行不通、本 skill 的 reference + dogfood + self-check 就是 multi-pass 設計」的根據。
>
