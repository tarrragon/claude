...
> **角色**：本卡是 `requirement-protocol` 的支撐型原則（principle）、被 SKILL.md「相關抽象層原則」段（#79）與 reference `decision-dialogue.md` 引用、是 `decide-later-as-valid-option` / `retrospective-multi-select-default` / `yes-no-binary-collapse` 等子卡的上層 meta。
>
...
五張子卡各自對應一個維度、互不重疊、合起來覆蓋 32 種決策對話組合。讀法建議：

- **遇到具體情境**：依步驟 1-5 找對應卡（例如「這個是反省題嗎？」→ 反省複選卡）
- **第一次接觸**：先讀本卡建立五維 mental model、再讀子卡學模板
