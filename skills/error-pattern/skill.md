...
**執行流程**：
1. **同義詞擴展**：比對 `references/synonym-map.md` 家族表，若用戶關鍵字命中任一家族的同義詞，展開為該家族全部變體的 multi-term OR grep（如 `grep -rli "confabul\|fabricat\|幻覺\|虛構\|腦補"`)。未命中任何家族時使用原始關鍵字。
2. 搜尋 `.claude/error-patterns/` 目錄下所有 `.md` 檔案
