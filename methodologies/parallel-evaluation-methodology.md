...
收集所有視角的發現，通過 Worth-It Filter 過濾，只保留值得行動的項目。
...
| 改善幅度 | 變更風險 | 決策 |
|---------|---------|------|
| 高（消除 bug/安全漏洞） | 任何 | 必須執行 |
| 高（顯著簡化） | 低 | 值得執行 |
| 高（顯著簡化） | 高 | 建立 Ticket，規劃執行 |
| 中（可讀性/維護性） | 低 | 值得執行 |
| 中（可讀性/維護性） | 高 | 跳過或延後 |
| 低（風格偏好） | 任何 | 跳過 |
...
- 如果需要超過 30 秒解釋「為什麼這值得改」，大概就不值得
- 多個視角同時發現的問題，可信度更高
- 只有一個視角發現且改善幅度為「中」的項目，降級處理
...
| G: 系統設計 | 術語不一致必須修正；交叉引用缺失必須補齊；風格偏好跳過 |
