...
>
> **陷阱（實測案例）**：勿因「交付物是計畫」誤判為研究性而用 spawned + --yes-spawned 強制完成——「產出計畫」不等於「問題已解決」。判別關鍵是**問題是否已解決**，非「是否產出了文件」。盤點計畫只是解法藍圖，清理執行才解決問題。清理延後時 ANA 跨 round 維持 in_progress（由 `stuck-anas` 追蹤），不強制本輪 complete。詳見 PC-091 v1.2.0。
