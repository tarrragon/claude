...
**列選項前提（副作用取捨）**：候選選項間存在副作用取捨（在不同維度上代價方向相反，例如某方案防護強度高但風險也高）時，禁止直接呈現直覺清單。

**Why**：直覺清單天然被單一程度軸限定，選項一旦列出即封閉解空間，且該封閉對決策者不可見——決策者只會看到自己列了三個選項，不會看到沒被列出的第四個；未經擴增即呈現時，被標為推薦的方案也可能只是基於未查證的基本率假設。

**Consequence**：使用者只能在被限定的清單內選擇；若推薦方案的假設被後續查證推翻，已耗費使用者一輪決策成本（某次文件索引接線方式選型即為此例：首輪三個選項可被一句話概括、未通過框架概括測試，且首輪標為推薦的方案基於未查證的基本率假設，被現實檢驗證偽）。

**Action**：識別到選項間存在副作用取捨時，先執行 `/wrap-decision` 的 W 階段（假選項偵測 + 框架概括測試 + 方案性質三類涵蓋），選項清單取自其產出再呈現 AUQ。處置取捨類選項的完整檢查清單見下方「選項空間檢查」節；判準 substance 權威來源見 `.claude/skills/wrap-decision/SKILL.md`「二元處置取捨」節（已擴充涵蓋設計選型類）。
...
| 建置 / 打包驗證 | 否（直接派發） |
| AC 實況驗證 | 否（直接派發） |
| 實機驗證（`/verify`、`/run` build-and-drive） | 否（**PM 前台執行**，非派發——見 `.claude/rules/core/pm-role.md`「實機驗證屬 PM 職責」） |
...
### 規則 6 附則：與 anti-paternalism 的適用邊界（流程路由 vs 價值取捨）

> **背景**：`.claude/skills/wrap-decision/references/anti-paternalism.md` 將 Recommended 標記列為暗黑模式（dark pattern），要求改以文字揭露傾向；本規則卻要求特定情境標記 `(Recommended)` 或依證據強度分級為 `(Recommended by WRAP)`。兩份文件字面各自讀作「一律禁止標記」與「特定情境必須標記」，需要顯性邊界避免臨場裁量。

**判別依據**：選項間的差異是否涉及使用者價值偏好（風格、風險承受度、個人化選擇，無客觀對錯的取捨）。

| 選項類型 | 判別特徵 | Recommended 標記 | 依循文件 |
|---------|---------|------------------|---------|
| 流程路由類 | 選項差異是「下一步做什麼」（例：Handoff vs 續做、派發方式、任務排序），有客觀機制支撐（如 Context 資源保護），不涉及個人偏好 | 可標記，依本規則分級 | 本規則規則 6 |
| 價值取捨類 | 選項差異涉及使用者價值偏好（方案選擇的風險承擔、個人化建議、無客觀對錯的取捨） | 禁止標記，改依 anti-paternalism 以文字揭露傾向 | anti-paternalism.md |

> 本表與 `.claude/skills/wrap-decision/references/anti-paternalism.md`「與 askuserquestion-rules 的適用邊界」判別表對稱；內容以該表為 substance 權威來源，未來修改請先改該表再同步本表。

**Why**：Recommended 標記把「有證據支撐的建議」與「作者主觀傾向」用同一視覺符號呈現。流程路由類選項中這無害——「Handoff 優先」的依據是 Context 資源保護這類客觀機制，不是替使用者決定其價值觀；價值取捨類選項中，標記會讓使用者的偏好被系統性導向某個選項，構成 anti-paternalism 定義的暗黑模式（位置偏誤與 confirmshaming 效果疊加）。

**Consequence**：缺邊界時兩種失敗同時可能發生：(1) 誤把價值取捨類選項套用本規則分級（例：個人化建議標 `(Recommended by WRAP)`），使用者的自主選擇被暗示為次優；(2) 誤把流程路由類選項套用 anti-paternalism 禁令（例：Handoff 選項改為純文字揭露、不標記），會與既有 Hook 強制層（acceptance-gate-hook 等）的 Recommended 格式要求衝突，且喪失本規則「省力路徑必須對齊決策品質」的既有設計。

**Action**：設計 AUQ 選項前，先問「這些選項的差異是否涉及使用者的價值偏好？」是 → 套用 anti-paternalism（不標記，改以文字揭露傾向）；否（純流程/執行路徑選擇）→ 套用本規則規則 6（依證據強度分級標記）。

> 對稱條款見 `.claude/skills/wrap-decision/references/anti-paternalism.md`「與 askuserquestion-rules 的適用邊界」章節。
...
選擇「記錄錯誤學習」後，執行捕獲時分流判準（記錄前必答；判別問句：「另一個專案的 session 讀到這段，能用嗎」）：

| 學習成果性質 | 判別 | 目的地 |
|------------|------|--------|
| 框架相關 | 替換專案名稱與檔案路徑後仍成立 | `.claude/error-patterns/` 或 `.claude/rules/` `.claude/methodologies/` `.claude/references/`（依內容性質見升級路徑表） |
| 專案相關 | 僅在本專案成立（架構、領域規則、工具鏈） | `docs/` 或 `CLAUDE.md` |
| 兩者皆非 | 僅在當前 session 成立 | 不記錄。ticket md 已承載執行脈絡 |

**強制要求**：分流判準必答，不得省略評估直接落筆；memory 不是任何分支的目的地。框架相關的錯誤學習類需先過根因成熟度門檻，完整判準見 `.claude/pm-rules/pm-quality-baseline.md` 規則 7。
...
> **來源**：W11-011（PM 在仍有 83 pending 時 AUQ 選項 3 提 `/version-release check`，description 附 disclaimer「顯然不時但可行」——前後矛盾等於自承假選項，違反 `.claude/references/power-asymmetry-rules.md` §5.6 機制 4：illusion of control）。
...
**呈現副作用取捨類選項前，選項清單必須取自 WRAP W 階段產出，不得直接使用直覺清單。**

副作用取捨的識別特徵：選項在回答「這個東西該留還是該拿掉」（典型形態為保留 / 回退 / 停用 / 刪除的組合，或「維持現狀 vs 改動程度」的程度軸），或選項為設計選型（如接線方式、架構形態）且在不同維度上代價方向相反（例如強度高者風險亦高）。兩者共享同一失效機制：選項清單由直覺產生、未經 W 階段擴增即呈現。

| 檢查項 | 正確行為 | 違規訊號 |
|--------|---------|---------|
| 選項來源 | 先跑 WRAP W 階段（反向框架 + 三類涵蓋），清單取自其產出 | 直覺浮出兩三個對立選項即直接列出 |
| 框架檢查 | 已問過「原框架的反面是什麼」並記錄答案 | 全部選項落在同一程度軸上 |
| 三類涵蓋 | 改造既有 / 零工具 / 新增工具三類至少檢視過，不適用者說明理由 | 選項全屬同一性質 |

**Action**：識別到正要列處置類選項，或代價方向相反的設計選型選項時停手，執行 `/wrap-decision` 的 W 階段，清單取自其產出。W 階段產出與直覺清單一致時照列即可——最低成本是一次檢查，不是一次重寫。處置涉及防護能力增減（移除守衛、降級閘門、停用檢查）或不可逆變更時，W 階段不足，須跑完整 WRAP。

**與選項前提檢查的分工**：前提檢查是減法（刪除前提不成立的選項），本檢查是加法（補上框架外的選項）。兩者都執行才構成完整的選項品質把關。

> 本檢查的 Why / Consequence / 升級條件理由 / 實證案例，substance 權威來源見 `.claude/skills/wrap-decision/SKILL.md`「二元處置取捨」節——本節與該節為同一判準的兩處落地，未來修改請以該節為準並同步本節。
...
| 副作用取捨類選項（處置類或設計選型類）未經 WRAP W 階段產出 | 停止，執行 `/wrap-decision` W 階段後重新產出 AUQ |
...
- .claude/skills/wrap-decision/references/anti-paternalism.md - Recommended 標記暗黑模式論述（規則 6 附則的對稱邊界說明）

---

**Last Updated**: 2026-08-10
**Version**: 3.17.0 — 規則 1 新增「列選項前提（副作用取捨）」子節：候選選項間存在副作用取捨（不同維度代價方向相反）時，列選項前須先跑 WRAP W 階段，三明示 + 案例佐證（案例敘事改描述性標籤，不引用專案層級 ticket ID，依規則 8 全禁原則）；「選項空間檢查」節識別特徵擴充涵蓋設計選型類（標題同步更名為「副作用取捨類」），與 wrap-decision SKILL.md「二元處置取捨」節（已同步擴充）保持一致；違規處理表對應項同步更名

**Version**: 3.16.0 — 消除與 wrap-decision SKILL.md 的 substance 重複：「選項空間檢查」節刪去與 SKILL.md「二元處置取捨」節逐字重複的 Why/Consequence/實證段（含其中一處「規則 5.6」簡寫，隨段落一併移除），改為路由指向該節（substance 權威）；規則 6 附則判別表新增權威標註，指向 anti-paternalism.md 對稱判別表為權威來源；另一處「規則 5.6」簡寫改為含路徑的完整指稱

**Version**: 3.15.0 — 場景 #16「雙通道記錄要求」改為「知識捕獲時分流」：移除 memory 作為強制寫入通道的規範，改用 `pm-quality-baseline.md` 規則 7 的三分流表（框架相關／專案相關／兩者皆非）與判別問句，逐字沿用維持三處一致（0.2.1-W3-089，承接 0.2.1-W3-083 用戶裁示）
**Version**: 3.14.0 — 新增「規則 6 附則：與 anti-paternalism 的適用邊界」：判別依據為選項差異是否涉及使用者價值偏好，流程路由類（可標記）與價值取捨類（禁止標記，改文字揭露）分表對照；對稱條款同步加入 anti-paternalism.md
