...
| 「git status 有我沒印象的變更」 | 同儕可定址時，發訊詢問與查 ghost 痕跡**同時**展開（訊息視為可能不返回，不循序等待；循序最壞情況：耗一則訊息與等待後仍須完整溯源）再決定對帳或 revert | 依同儕回報直接對帳/派發/revert（回報僅線索，決策仍須痕跡查證） |

**分工線**：詢問同儕取得線索（記錄平面，起點用）；ghost 痕跡查證取得結論（世界平面，決策用，不可省）。與 `parallel-dispatch.md`「跨 session 同儕沉默時的接管判準」的「不採信對方回報」一致，不衝突。
...
**判定順序**（PC-BAL-001）：canonical 常數（建立端）＞ 驗證端清單 ＞ 文件描述。三者不一致時以建立端為準；`canonical-schema-consistency-check-hook.py`（SessionStart 觸發）自動比對建立端與下游清單並輸出具體差異項，可作為快速查證管道，不需人工逐檔比對。
