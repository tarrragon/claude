...
| 指標 | 觸發條件 | 判定 |
|------|---------|------|
| 預期修改檔案數 | > 5 個（即 6 個以上） | 必須拆分 |
| 認知負擔指數 | > 10 | 必須拆分 |
| 跨架構層級數 | > 2 層 | 必須拆分 |
| 策略文件中的獨立模組數 | > 1 個且無交叉依賴 | 建議拆分 |

**無需拆分**：上述觸發條件均未命中（檔案 <= 5、負擔 <= 10、層級 <= 2、單一模組或有交叉），且修改目的單一明確時，由單一代理人執行，無需拆分。
...
**Version**: 2.9.0 - 3b 拆分決策表合併：消除拆分觸發和豁免條件分散兩張表格的邊界模糊（0.1.0-W47-003）
