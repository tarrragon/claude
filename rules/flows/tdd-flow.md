...
**拆分決策表**（基於 Phase 3a 策略文件產出，任一觸發即拆分）：

| 指標 | 觸發條件 | 判定 |
|------|---------|------|
| 預期修改檔案數 | > 5 個（即 6 個以上，含測試檔案） | 必須拆分 |
| 認知負擔指數 | > 10 | 必須拆分 |
| 跨架構層級數 | > 2 層 | 必須拆分 |
| 測試檔案數 | > 3 個 | 建議拆分 |
| 預估 context 消耗 | > 20K tokens（含設計文件讀取 + 實作 + 測試） | 必須拆分 |
| 策略文件中的獨立模組數 | > 1 個且無交叉依賴 | 建議拆分 |

**無需拆分**：上述觸發條件均未命中（檔案 <= 5、負擔 <= 10、層級 <= 2、測試檔案 <= 3、context <= 20K、單一模組或有交叉），且修改目的單一明確時，由單一代理人執行，無需拆分。

> **注意**：測試檔案計入修改檔案數，不可視為輕量級。每個測試檔案包含 fixture 設定、多個測試案例和斷言邏輯，其 context 消耗與實作檔案相當。

### Context 預算評估（PM 派發前，建議）

PM 在派發 Phase 3b 代理人前，應根據 Phase 3a 策略文件估算 context 消耗，避免代理人 context 耗盡導致工作中斷。

**估算公式**：

```
預估 context = 設計文件讀取 + 程式碼實作 + 測試執行 + 緩衝
```
...
| 階段 | 估算方式 | 參考值 |
|------|---------|-------|
| 設計文件讀取 | Phase 1/2/3a 文件總量 | 每份約 2-3K tokens |
| 程式碼實作 | 預期新增/修改行數 | 每 100 行約 1K tokens |
| 測試執行 | 測試檔案數 x 平均測試量 | 每個測試檔約 1K tokens |
| 緩衝 | 工具輸出、錯誤重試等 | 總量的 30% |

**超過 20K tokens 時**：必須拆分為多個子任務，確保每個子任務的 context 消耗在代理人可用範圍內。
...
**Version**: 2.11.0 - 3b 拆分決策表刪除冗餘「程式碼實作行數」指標，保留 context 消耗作為工作量唯一評估入口（0.1.1-W13-004）
