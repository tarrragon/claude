...
## agent-writes-to-ticket 紀律（W8-038）

> **用途**：派發 agent 時，在 prompt 明示要求 agent 以 `ticket track append-log` 把執行狀況寫入 ticket；PM 驗收讀 ticket 章節，不信任 agent return text。
>
> **設計依據**：W8-038 fork-base 實驗暴露的反模式——首次探針依賴 agent return text，輸出整個遺失，並行第二視角結論也因此丟失（用戶 2026-06-07 當日指正之實例）。return text 脆弱、不持久、不可獨立驗證；ticket 是既有的持久結構化機制。

### Why / Consequence / Action

**Why**：agent return text 是一次性、脆弱、無法獨立查證的回傳通道——輸出可能整段遺失、被截斷、或在並行派發中丟失（W8-038 二度實證）。`ticket track append-log` 寫入的內容持久落地於 ticket md，PM 可從主 repo 獨立 Read 驗證，不依賴 agent 是否「成功回傳」。

**Consequence**：PM 信任 return text 驗收會踩兩個坑：(1) return text 遺失時 PM 誤判 agent 失敗或無產出，重派浪費資源；(2) return text 與實際 ticket / commit 狀態不一致時（agent 宣稱完成但未寫入），PM 在 handoff 時才發現缺口，回溯成本高。

**Action**：

| 對象 | 動作 |
|------|------|
| 派發 prompt | 明示要求 agent 用 `ticket track append-log <id> --section "<章節名>"`（heredoc 傳長文字）寫入執行狀況、設計決策、測試結果 |
| PM 驗收 | 讀 ticket 對應章節（`ticket track full <id>` 或 Read ticket md）+ 查 git log / commit，**不以 return text 為驗收依據** |
| 驗證手法 | 需要客觀證據時（如 fork base、commit parent），從主 repo `git` 獨立反推，不依賴 agent 自述 |

### prompt 插入範本

在派發 prompt 的「## 任務」或「## 執行」段加入：

```markdown
執行過程把狀況寫入 ticket（不靠 return text 回報）：
  ticket track append-log <ticket_id> --section "Solution"（或對應章節）
完成後 PM 會讀 ticket 章節 + git log 驗收，return text 僅作摘要。
```

### 與既有實作類 agent 自律責任的關係

`.claude/rules/core/agent-definition-standard.md`「執行責任：Ticket body 填寫」已要求實作類 agent 主動填 body；本紀律是其在「派發 prompt 層」的強制提醒，並擴及非實作類 agent（如分析 / 探針任務）的執行狀況回報。兩者互補：agent 定義是自律主責，派發 prompt 是派發時的明示提醒。
...
>
> **前置依賴（W8-038）**：本指引的 `git merge main` 只在 local main 已 push 到 origin 時可靠。harness worktree fork base = origin/main，origin 落後時 worktree 內無對應 git object，merge 補不回 local 未 push 的工作。故 worktree 派發前須先完成 push-before-dispatch（見 `.claude/pm-rules/parallel-dispatch.md`「worktree 派發前置：push-before-dispatch」）。
...
### 回收 worktree 產出：禁用 naive merge，改外科抽取（W8-038）

> **用途**：worktree agent 完成後 PM 回收產出時，修正既有有害提醒——origin 落後 / fork 分歧時，hook 自動建議的 `git merge worktree-…` 會 revert 本地未推工作。

**Why**：worktree fork 自 stale origin/main 時，其分支不含 local main 已有但未 push 的 commit。對這種落後分支執行 `git merge worktree-<branch>`，git 會把「local 有、worktree 沒有」的檔案狀態視為「worktree 刪除了這些變更」，merge 結果反而 revert 掉本地未推工作（W8-034 觀察 2 實證）。

**Consequence**：PM 照 hook 提示盲目 `git merge worktree-…`，local main 上未 push 的工作被靜默回退；diff 看似「合併成功」實則丟失本地 commit，回溯與重做成本高。

**Action**：

| 情境 | 正確做法 | 禁止 |
|------|---------|------|
| origin 落後 / worktree 分支與 local main 分歧 | 外科抽取：`git checkout <worktree-branch> -- <目標檔>` 只取 agent 實際修改的檔案，再於 local main 檢視 + commit | `git merge worktree-<branch>`（會 revert local 未推工作） |
| worktree fork 自最新 origin/main 且無分歧（已執行 push-before-dispatch） | ff-merge 安全：`git merge worktree-<branch>` 乾淨 | — |

判別準則：執行回收前先 `git rev-list --count origin/main..main`。非 0（origin 落後）→ 必走外科抽取；== 0 且 worktree 自最新 origin/main fork → ff-merge 可用。push-before-dispatch（見 `.claude/pm-rules/parallel-dispatch.md`）是讓回收能走 ff-merge 的前置條件。
...
**Version**: 1.7.0 — 新增「agent-writes-to-ticket 紀律（W8-038）」章節（append-log 寫執行狀況、PM 驗收不信任 return text）；worktree 派發 base 同步指引（W1-035）新增 W8-038 前置依賴註記（merge main 僅在 local main 已 push 時可靠）+ 「回收 worktree 產出：禁用 naive merge，改外科抽取（W8-038）」子章節（origin 落後時 git merge worktree-… 會 revert 本地未推工作，改 git checkout <branch> -- <檔>）（0.31.1-W8-039，source W8-038）
