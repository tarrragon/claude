...
| DOC-GPD-002 | 資產間引用單向累積，既有資產出度隨時間下降且不自我暴露 | 中 | 0.1.0 |
| DOC-GPD-003 | 敘述中實例化可變計數，集合增減一員即全量失效 | 中 | 0.1.0 |
| DOC-GPD-004 | 標籤壓縮省下的字數固定，展開成本乘以讀者數與引用點數 | 中 | 0.1.0 |
| DOC-GPD-005 | 判準以類別名為鍵，類別後來獲得性質不同的新成員，新成員繼承舊裁決而被誤放行 | 中 | 0.1.0 |
...
| ARCH-BAL-021 | 撞號的版本號使分叉看似已同步，偵測到分叉的警告不阻擋寫入 | 高 | v0.2.1 |
| ARCH-BAL-022 | 同一名稱同時存在於兩個同步狀態，讀者把分歧歸因至錯誤的一邊；工具已附免責句仍誤讀 | 中 | v0.2.1 |
| ARCH-BAL-023 | 寫入端與消費端各自自洽，識別欄位缺共同格式約定且缺席無訊號 | 中 | v0.2.1 |
...
| IMP-BAL-006 | 同一排除集合在同一同步方向的兩條消費路徑處置相反——套用與否由呼叫端各自決定而非集合定義強制 | 高 | v0.2.1 |
...
| PC-BAL-050 | 參數敏感決策以單點量測作主論據，數值再被措辭升級為分佈宣稱 | 高 | — |
| PC-BAL-051 | AskUserQuestion 選項措辭正確但未附量化後果，用戶在資訊不完整下裁示不可逆操作 | 高 | 0.2.1 |
| PC-BAL-052 | 技術判準在並行期間誕生，未回頭同步給仍在執行的同批票 | 中 | 0.2.1 |
| PC-BAL-053 | 依檔名查證失效引用，對「功能被合併進不同名檔案」為盲，產出功能缺口假結論 | 高 | v0.2.1 |
| PC-BAL-054 | 分析票只落地判定不落地逐筆清單，下游票依賴不存在的資料 | 高 | v0.2.1 |
| PC-BAL-055 | 派發到 claim 的真空期未落票，pending 被誤讀為無人處理 | 中 | 0.2.1 |
| PC-BAL-056 | 以增加審查輪次尋找潛伏缺陷，而該類缺陷的發現率取決於操作多樣性 | 高 | 0.2.1 |
| PC-BAL-057 | 安全網與它保護的判斷共享同一個可能失準的輸入，故兩者同時失效 | 高 | 0.2.1 |
| PC-BAL-058 | 某類別只有一個已知成員時，問題的範圍無法被觀察，答案自動繼承提問的範圍 | 高 | 0.2.1 |
...
| PC-GPD-005 | 版號是單調計數器不表達分叉，兩個 consumer 各自前進一步即撞號，內容雜湊只證明不同不證明誰新 | 高 | 0.1.0 |
| PC-GPD-006 | 「各專案自建」目錄跨 consumer 整包複製，夾帶他方 ticket 編號與指向不存在檔案的索引 | 中 | 0.1.0 |
| PC-GPD-007 | 樣本中一致的部分被讀成機制，對不上的那一格被歸類為補充證據而非機制的邊界 | 中 | 0.1.0 |
| PC-BAL-059 | 驗收子項被下游票承接後上游票 acceptance 未同步，使上游無人能合法收尾 | 中 | 0.2.1 |
| PC-BAL-060 | 逃生閥只解除發放它的那道閘門，其餘閘門的判準不隨之放行 | 高 | 0.2.1 |
| PC-BAL-061 | 驗收只跑 where.files 對應測試，同函式的其他測試檔在環境假失敗遮蔽下漏檢 | 高 | 0.2.1 |
| PC-BAL-062 | 連續訊息各自正確，合成後的方向與發送者意圖相反 | 中 | 0.2.1 |
| PC-BAL-063 | 雙向同步中推送參與者對自己推送過的內容天然免疫，缺陷只咬純消費者 | 中 | v0.2.1 |
| PC-BAL-064 | 判準的驗證取樣落在單一維度的單一格，適用範圍被誤讀為全域 | 高 | v0.2.1 |
| PC-BAL-065 | 修法本身是它所要修的缺陷的另一個實例 | 中 | v0.2.1 |
| PC-BAL-066 | 同一產出中查證與猜測並列，查證的部分使猜測的部分看起來也經過查證 | 中 | v0.2.1 |
| PC-BAL-067 | 沉默有多種成因而其中一種被預設，觀測與工具皆正確但結論超出觀測的射程 | 中 | v0.2.1 |
| PC-BAL-068 | 派發期間更新規範，執行者以開場讀入的版本收尾且雙方皆無訊號 | 中 | v0.2.1 |
| PC-BAL-069 | 執行者換了量尺量門檻，得出與規範相反的結論而報告外觀相同 | 中 | v0.2.1 |
| PC-BAL-070 | 探針執行體自動載入專案規則，術語探針量到的是詞頻不是語域 | 中 | v0.2.1 |
| PC-BAL-071 | 修法批次自身無人審查——授權它的那一輪審查在它之前執行，結構上看不到它引入的新缺陷 | 中 | 0.2.1 |
| PC-BAL-072 | 單一 --force 收掉多道獨立閘門，而關鍵判讀行印在成功訊息之前，操作者只讀最後一行 | 高 | 0.2.1 |
| PC-BAL-073 | 以專案名索引行為，使同專案的多個 session 被歸納成單一行動者的重複犯錯 | 中 | 0.2.1 |
| PC-BAL-074 | 以內容經由哪條通道抵達推斷誰是作者——通道不承載作者身分 | 中 | 0.2.1 |
| PC-BAL-075 | 全樹 overlay 推送前未先拉取，靜默移除他方已在共用庫的內容 | 高 | 0.2.1 |
