...
Claude Code 工具（Edit/Write）傳入的 `file_path` 是**絕對路徑**（如 `/Users/mac-eric/project/ccsession/.claude/rules/core/decision-tree.md`），但 Hook 的豁免比對邏輯只做 `lstrip("/")`，結果是 `Users/mac-eric/...`，永遠不會匹配 `.claude/` 前綴。
