...
## 更新（2026-07-31，探針證實後）：自我修正——推測碰巧正確，不等於當時該當事實引用

1.4.0-W1-023 事後取得探針逐字證據，結果與本檔上方分析的方向相反：**W1-015 的「字面引數 token `complete` 被判為不安全的 shell builtin 樣式」這句診斷是對的**。逐字阻擋原文為：
...
守衛確實將命令字串中的 `complete` token 判定為「透過 complete 這個 shell builtin 執行一段字串」（"runs a string through complete"）而拒絕，與 W1-015 原始診斷語意精確對應。完整逐字證據與判據見 `.claude/error-patterns/process-compliance/PC-SCLK-002-agent-obfuscation-bypass-of-sandbox-guard.md`「更新：機制描述已由逐字證據證實」章節。

**這件事本身需要被本檔正確吸收，而不是被當作反例悄悄放過**：上一節「可信度分層」把這句診斷評為「低可信度——推測性歸因」，探針證實後，這個**可信度評級的判斷過程沒有錯**，即使**它評級的對象後來被證實為真**。原因是可信度評級評的是「撰寫當下有沒有逐字證據支撐」，不是「這句話最終對不對」——W1-015 撰寫當下確實沒有附逐字原文，這件事在探針結果出來之後依然成立，不會因為猜對了就回溯性地變成「當時有證據」。

用一個對照更精確：**若丟一枚硬幣猜正反，猜中不代表這是一次有依據的判斷**。早期診斷之所以在方法論上不合格，不是因為它錯了，而是因為它在沒有逐字證據的情況下就被寫成確定語氣的結論，並被後續的相關回報直接引用——甚至生成了互斥版本（「跨 worktree git 操作」），**兩個版本不可能同時為真，但兩者在各自報告當下都被寫成同樣篤定的語氣**。這正是本模式要防的：篤定語氣與證據強度脫鉤，讀者無法從文字本身分辨這是「已驗證」還是「合理推測」。

**本模式（PC-SCLK-004）的核心主張不受這次結果影響，反而因此更有說服力**：防護 A（附逐字原始輸出）要求的不是「不准推測」，而是「推測要標成推測」。若 W1-015 當初依防護 A 的格式寫成「阻擋發生，具體機制未知，疑似與 `complete` token 有關；已附逐字輸出供後續診斷比對」，則探針證實後只需要把「疑似」改成「已證實」，不需要任何額外調查成本；正因為它省略了這一步、直接寫成確定結論，才讓 1.4.0-W1-023 花了一整輪重現實驗與 repo 全文檢索去驗證一個（結果為真但）當時無憑無據的說法。**證據不足與結論錯誤是兩件不同的事，本檔記錄的是前者，這次事後驗證恰好示範了兩者可以完全獨立發生。**
