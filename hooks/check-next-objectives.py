...
def main():
    ...
    todolist_file = Path(project_root) / "docs" / "todolist.md"
    ...
    if not todolist_file.exists():
        print("[ERROR] 錯誤：找不到 todolist.md 檔案", file=sys.stderr)
        return 1

    if not pubspec_file.exists():
        print("[ERROR] 錯誤：找不到 pubspec.yaml 檔案", file=sys.stderr)
        return 1
    ...
    if not version_match:
        print("[ERROR] 無法從 pubspec.yaml 提取版本", file=sys.stderr)
        return 1
    ...
    if series_status == "FULLY_COMPLETED":
        ...
        print(f"[WARNING] 版本系列 {version_series} 的目標尚未完全達成", file=sys.stderr)
        return 2
    else:
