...
def is_version_release_commit(tool_input: dict) -> tuple:
    """檢查是否為版本發布相關的 commit。"""
...
def is_via_version_release_script() -> bool:
    """檢查是否透過 version_release.py 執行。"""
    return os.environ.get("VERSION_RELEASE_SCRIPT") == "1"


def check_pubspec_version() -> tuple[bool, str]:
    ...
    if not pubspec_match:
        return False, "無法從 pubspec.yaml 解析版本號"
    ...
    if not changelog_match:
        return False, "無法從 CHANGELOG.md 解析版本號"
    changelog_version = changelog_match.group(1)

    # 比較版本
    if pubspec_version != changelog_version:
        return False, f"版本不一致: pubspec.yaml={pubspec_version}, CHANGELOG.md={changelog_version}"
...
def main():
    ...
    # 檢查是否為版本發布相關的 commit
    is_version_commit, version_tag = is_version_release_commit(tool_input)
    ...
    if not is_via_version_release_script():
        # 不強制攔截，改為警告提示
        warning_msg = f"""
...
"""
    ...
    version_ok, version_info = check_pubspec_version()
    if not version_ok:
