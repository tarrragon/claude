...
**Version**: 5.1.0 — 對齊 compositional-writing v0.17.0：keyword bank 新增歸因語氣 grep（`承認|暴露了|證明了失敗|被迫`）+ 否定起手定義句 pattern（`不是.{0,30}而是`）；輪 8-10 描述補恐嚇式語氣 / 歸因語氣兩個新 design gap。代理人透過 @-import auto-load SKILL.md 自動獲得新規範，本次僅更新版本記錄。
